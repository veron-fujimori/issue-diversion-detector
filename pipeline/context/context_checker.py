...
class ContextCheckResult:
    ...
    confidence: float
    reasoning: str
    grounded: bool = True   # False kalau ternyata gak beneran search
...
def run(alert: Alert, rising_topics: list[str]) -> ContextCheckResult:
    ...
    try:
        ...
        # Kalau model gak beneran search, jangan percaya hasilnya untuk suppression
        if not response.grounded:
            logger.warning(
                f"context_checker | alert_id={alert.id} | "
                f"NOT grounded (no web_search_call) — fail-safe, tidak suppress"
            )
            return ContextCheckResult(
                independent_event=False, confidence=0.0,
                reasoning="LLM tidak melakukan web search aktual", grounded=False,
            )
        ...
        # Gagal cek = fail-safe ke "tidak independen" -> skor TIDAK disuppress
